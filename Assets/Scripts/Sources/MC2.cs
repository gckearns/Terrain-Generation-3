...
//-----------------------------------------------------------------------------
{
    ...
    public MarchingCubes2(int size_x = -1, int size_y = -1, int size_z = -1)
    {
        _originalMC = false;
        _ext_data = false;
        _size_x = size_x;
    ...
    public Vertex2? GetVertex(int i) { if (i < 0 || i >= _nverts) return null; return _vertices[i]; }
    ...
    public Vertex2[] vertices { get { return _vertices; } }
    ...
    public void Run(float iso = 0f)
    {
        ...
        stopwatch.Stop();
        Debug.LogFormat("Old-New Marching Cubes 33:ran in {0} secs.\n",
    ...
    protected void test_vertex_addition()
    {
        if (_nverts >= _Nverts)
        {
            Vertex2[] temp = (Vertex2[])_vertices.Clone();
            _vertices = new Vertex2[_Nverts * 2];
    ...
    protected Vertex2[] _vertices;  /**< vertex   buffer */
